...
from nengo.utils.numpy import is_array_like, is_integer
...
def get_nengo_object(root, attr_list):
    attr = attr_list[0]

    if is_integer(attr) or isinstance(attr, slice) or isinstance(attr, list):
        return root[attr]

    if hasattr(root, attr):
        obj = getattr(root, attr)
        if len(attr_list) > 1:
            return get_nengo_object(obj, attr_list[1:])
        else:
            return obj
    else:
        warn(f"Spaun probe - Could not find object " +
             f"'{root.__class__.__name__}.{attr_list}'")
        return None
...
class SpaunProbeConfig(object):
    ...
    def probe_value(self, probed_obj, synapse=0.005, vocab=None, label=None):
        if isinstance(probed_obj, str):
            ...
            if is_array_like(probed_obj):
                probed_obj = get_nengo_object(probed_obj[0], probed_obj[1:])

            if probed_obj is None:
                return None
    ...
    def write_config_to_file(self):
        config_data = {"graph_list": self.graph_list,
                       "sp_dim": self.v.sp_dim,
    ...
    def write_simdata_to_file(self, sim, experiment):
        # Get and process experiment stimulus sequence list.
        # Note that handwritten digits are tuple, so they need to be made into
        # string representations.
        stim_seq_list = experiment.stim_seq_list
        for i, item in enumerate(stim_seq_list):
            if isinstance(item, tuple):
                stim_seq_list[i] = str(item)

        # Generic probe data (time and stimulus sequence)
        probe_data = {"trange": sim.trange(),
                      "stim_seq": stim_seq_list,
...
class ProbeCfgVisOnly(SpaunProbeConfig):
    def initialize_probes(self):
        ...
        pvs1 = self.probe_value([self.m, "vis", "out_main"], vocab=self.v.vis_main)
        pvs2 = self.probe_value([self.m, "vis", "out_neg_attn"])
...
class ProbeCfgMtrOnly(SpaunProbeConfig):
    def initialize_probes(self):
        ...
        if hasattr(self.m, "mtr"):
            ...
            pmt10 = self.probe_value([net, "target_diff_norm_out"], label="tgt_diff_norm")
...
class ProbeCfgDefault(SpaunProbeConfig):
    def initialize_probes(self):
        ...
        if hasattr(self.m, "vis") and True:
            ...
            pvs1 = self.probe_value([net, "out_main"], vocab=self.v.vis_main,
                                    label="Vis Vcb")
            pvs2 = self.probe_value([net, "out_neg_attn"], label="Neg Attn")
            pvs3 = self.probe_value([net, "vis_classify_utilities"],
                                    label="Vis Utils")
            pvs3i = self.probe_value([net, "vis_classify", "inhibit"],
                                     label="Vis AM Inhibit")
            pvs4 = self.probe_value([net, "out_vis_mb"], label="Vis MB",
                                    vocab=self.v.vis)
            pvs4b = self.probe_value([net, "out_vis_mb"], label="Vis MB")
            ...
            pvs6m = self.probe_value([net, "vis_main_mem", "mem", "output"],
                                     vocab=self.v.vis_main)
            pvs7 = self.probe_value([net, "vis_net", "to_classify_output"],
                                    synapse=0.03)
        ...
        if hasattr(self.m, "ps") and True:
            net = self.m.ps
            pps1 = self.probe_value([net, "out_task"], vocab=self.v.ps_task,
                                    label="ps task")
            pps2 = self.probe_value([net, "out_state"], vocab=self.v.ps_state,
                                    label="ps state")
            pps3 = self.probe_value([net, "out_dec"], vocab=self.v.ps_dec,
                                    label="ps dec")
            ...
            pps16 = self.probe_value([net, "out_action"],
        ...
        if hasattr(self.m, "trfm") and \
           not isinstance(self.m.trfm, TransformationSystemDummy):
            ...
            ptf4 = self.probe_value([net, "out_out"], vocab=mem_vocab)
            ...
            #                         label="NormB In")
            ptf10 = self.probe_value([net, "compare", "dot_prod"], label="Cmp DP")
        ...
        if hasattr(self.m, "trfm") and \
           not isinstance(self.m.trfm, TransformationSystemDummy):
            net = self.m.trfm
            ptf5 = self.probe_value([net, "am_trfms", "pos1_to_pos"], vocab=self.v.pos)
            ptf6 = self.probe_value([net, "am_trfms", "pos1_to_num"], vocab=self.v.item)
            ptf7 = self.probe_value([net, "am_trfms", "num_to_pos1"], vocab=self.v.pos1)
            ptf8 = self.probe_value([net, "am_trfms", "pos_to_pos1"], vocab=self.v.pos1)
            ptf14 = self.probe_value([net, "am_trfms", "frm_compare"],
                                     vocab=self.v.ps_cmp, label="Frm Cmp")
            ptf15 = self.probe_value([net, "am_trfms", "compare_out"],
        ...
        if hasattr(self.m, "reward") and True:
            prw1 = self.probe_value([self.m, "bg", "input",
                                     np.s_[:len(cfg.learn_init_transforms)]])
        ...
        if hasattr(self.m, "dec") and True:
            ...
            pde15 = self.probe_value([net, "out_know"])
            pde16 = self.probe_value([net, "out_unk"])
            pde18 = self.probe_value([net, "out_stop"])
            # pde19 = self.probe_value([net, "am_th_utils"])
            # pde20 = self.probe_value([net, "fr_th_utils"])
            pde21 = self.probe_value([net, "out_out"], vocab=self.v.mtr)
...
#             pvs1 = self.probe_value([self.m, vis.out_main, vocab=vis_vocab,
#                                     label="Vis SP")
