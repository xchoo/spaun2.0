...
def Spaun():
    ...
    # Create modules. Note that modules are created here (and not within the
    # model context) to support multi-process Spaun, where each module is
    # simulated separately
    if cfg.has_vis:
        module = Vision()  # 10
        modules[module.id_str] = module
    if cfg.has_ps:
        module = ProdSys()  # 11
        modules[module.id_str] = module
    if cfg.has_reward:
        module = RewardEval()  # 12
        modules[module.id_str] = module
    if cfg.has_enc:
        module = InfoEnc()  # 13
        modules[module.id_str] = module
    ...
    if cfg.has_mtr:
        ...
        modules[module.id_str] = module
    if cfg.has_instr:
        module = InstrProcess()  # 18
        modules[module.id_str] = module

    if cfg.multi_process:
        # Reconfigure Spaun modules for multi-processing support
        proc_count = 0
        for id_str, module in modules.items():
            # Skip multi-process conversion for specific modules. All skipped modules
            # will be added to the "main" Spaun  network
            if id_str in ["reward"]:
                continue

            # Create sub-spaun model with the select individual modules
            model = spa.SPA(label=f"SubSpaun[{proc_count}] ({id_str})", seed=cfg.seed)
            with model:
                module.add_multi_process_support()
                model.add(module)
                setattr(model, id_str, module)
            spaun_networks.append(model)

            # Update modules dictionary with multi-process module hub
            modules[id_str] = module.get_multi_process_hub()

            # Increment proc count
            proc_count += 1

    # Create Spaun SPA modle
    ...
    with model:
        ...
        # Add the stimulus modules (if selected)
        if cfg.has_stim:
            model.stim = Stimulus()  # 0
            model.instr_stim = InstrStimulus()  # 1
            model.monitor = Monitor()  # 2
        ...
        if hasattr(model, "vis"):
            model.vis.setup_module_connections(model)
        if hasattr(model, "ps"):
            model.ps.setup_module_connections(model)
            # Modify any "channel" ensemble arrays to have
            # get_optimal_sp_radius radius sizes
            for net in modules["ps"].all_networks:
        ...
        if hasattr(model, "thal"):
            pass
        if hasattr(model, "reward"):
            model.reward.setup_module_connections(model, model.learn_conns)
        if hasattr(model, "enc"):
            model.enc.setup_module_connections(model)
        if hasattr(model, "mem"):
            model.mem.setup_module_connections(model)
        if hasattr(model, "trfm"):
            model.trfm.setup_module_connections(model)
            # Modify any "channel" ensemble arrays to have
            # get_optimal_sp_radius radius sizes
            for net in model.trfm.all_networks:
                if net.label is not None and net.label[:7] == "channel":
                    raise RuntimeError("CHANNEL DETECTED IN TRFM")
                    for ens in net.all_ensembles:
                        ens.radius = cfg.get_optimal_sp_radius()
        if hasattr(model, "dec"):
            model.dec.setup_module_connections(model)
        if hasattr(model, "mtr"):
            model.mtr.setup_module_connections(model)
        if hasattr(model, "instr"):
            model.instr.setup_module_connections(model)
        if hasattr(model, "monitor"):
            model.monitor.setup_module_connections(model)

        spaun_networks.append(model)

    return spaun_networks
