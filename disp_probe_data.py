...
parser.add_argument(
    ...
    help="Supply to save the graph of plot data to file.")
parser.add_argument(
...
parser.add_argument(
    "--data-dir", type=str, default=os.path.join(cur_dir, "data"),
    help="Directory to store output data.")
parser.add_argument(
    "--save-dir", type=str, default=os.path.join(cur_dir, "plots"),
    help="Directory to store saved plots.")
parser.add_argument(
    "--legend-pos", type=str, default="best",
...
save_grphs = args.savegrph
show_grphs = args.showgrph
show_io = args.showiofig
show_anim = args.showanim

# If no `show` or `save` options are provided, default to showing graphs
if not (show_grphs or show_io or show_anim or save_grphs):
    show_grphs = True
...
net_label = config_data["net_label"].item()
...
if show_grphs or save_grphs:
    ...
    for n, fig in enumerate(grph_list):
        ...
        # Add "close all windows on one close" callback
        if show_grphs:
        ...
        if len(title_list[n]) > 0:
            plt.suptitle(f"{net_label} - {title_list[n][-1]}")
        ...
        if save_grphs:
            plot_filename = f"{net_label}_{title_list[n][-1]}.png"
            out_fp = os.path.join(args.save_dir, plot_filename)
            f.savefig(out_fp, bbox_inches="tight", dpi=300)
...
# Open figures
if show_grphs or show_io or show_anim:
    plt.show()

# Close probe data files
probe_data.close()
