...
class SocketCheckAliveThread(threading.Thread):
    def __init__(self, socket_class):
        ...
        self.recv_socket = socket_class
        self.name = str(int(time.time()))

    def run(self):
        # Keep checking if the socket class is still being used.
        while (time.time() - self.recv_socket.last_active <
               self.recv_socket.max_idle_time_current):
            time.sleep(self.recv_socket.alive_thread_sleep_time)
        # If the socket class is idle, terminate the socket
        self.recv_socket._close_socket()
    ...
    def run(self, t, x=None):
        ...
        if (self.is_sender):
            if (t - self.last_packet_t >= self.dt_remote):
                self.send_socket.sendto(self.pack_packet(t, x),
        ...
        if (self.is_receiver):
            ...
            while (not found_item):
                try:
                    packet, addr = self.recv_socket.recvfrom(self.max_len)
